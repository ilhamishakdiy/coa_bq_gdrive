...
def _resolve_drive_destination(
    ...
    supplied_values = (country_code, year_id, month_id)
    if not any(value is not None for value in supplied_values):
        return parent_folder_id, None

    if not all(value is not None for value in supplied_values):
        raise ValueError(
            "COUNTRYCODE, YEARID, and MONTHID must be provided together "
            "when overriding the Google Drive folder and filename."
        )

    safe_country_code = _validate_country_code(country_code or "")
    ...
    drive_file_name = _build_drive_file_name(
        country_code=safe_country_code,
        year_id=int(year_id or 0),
        month_id=int(month_id or 0),
    )
...
def _parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        type=int,
        help="Year ID used for the final Drive filename.",
    )
    parser.add_argument(
        "--monthid",
        "--month-id",
        ...
        default=None,
        type=int,
        help="Month ID used for the final Drive filename.",
    )
